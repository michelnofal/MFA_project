...
intra.dat <- read.table("first_timecourses/intracellular_data_timecourse2.csv", header=TRUE, sep=",")
# import extracellular data
extra.dat <- read.table("first_timecourses/extracellular_data_timecourse2.csv", header=TRUE, sep=",")
...
growth.func <- function(data) {
  ...
  growth.plot <- ggplot() + 
    ...
    scale_y_continuous(limits = c(0, ceiling(growth.max$max)))
...
aa.lab.func <- function (intra.data, extra.data) {
  ...
  aa.plots <- ggplot () + 
    ...
    geom_line(data=aa.preds, aes(x=time.pt, y=predicted, color=compound)) + 
...
calc.uptake.rates <- function (aa_table, aa.dt, growth.preds, growth.curve.params) {
  ...
  pve.plot <- ggplot(pve.aug) + geom_point(aes(x=predicted,y=umol,color=compound)) + 
    geom_line(aes(x=predicted, y=pred.umol, color=compound)) +
    scale_color_hue(c=60, l=70) + 
...
# calculate flux using MFA equation
# plot flux estimates from different aas
# plot influx of amino acids through monomeric uptake vs through protein catabolism 
