...
lin.model <- lm(frac.dt2$frac ~ frac.dt2$time.pt)
summary(lin.model)

pred.lm <- function (x, mod) {
  as.numeric(mod$coefficients[2]*x) + as.numeric(mod$coefficients[1])
}
frac.dt2 <- frac.dt2 %>% mutate(lm.frac = pred.lm(time.pt, lin.model))

quad.model <- predict(quadratic.model)
