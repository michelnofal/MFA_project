...
final.umol.unlab.2 <- merge(extra.umol.2, all.max) %>% filter(loc == "extracellular", time.pt %in% c(0,max)) %>% 
  group_by(compound, time.pt) %>% summarize(mean.umol.unlab=mean(umol.unlab))
final.umol.unlab.2$time.pt[final.umol.unlab.2$time.pt == 0] = "time0"
final.umol.unlab.2$time.pt[final.umol.unlab.2$time.pt != "time0"] = "endtime"
final.umol.unlab.2 <- final.umol.unlab.2 %>% spread(time.pt, mean.umol.unlab)
final.umol.unlab.2 <- final.umol.unlab.2 %>% mutate(net.umol.unlab = endtime - time0) %>% select(-time0, -endtime)
...
growth.tidy.spread.2 <- merge(growth.tidy.spread, growth.max) %>% filter(expt == 2) %>% select(-expt)
...
product.integrals <- merge(intra.tidy.spread, growth.tidy.spread.2)
...
eqn.vars.2 <- merge(alpha.df, merge(final.umol.unlab.2, uptake.rates.2))
eqn.vars.2$growth.integral <- (growth.integrals %>% filter(expt == 2))$growth.integral
eqn.vars.2 <- merge(eqn.vars.2, product.integrals %>% select(compound, product.integral))
# plug variables into equation
eqn.vars.2$umol.prot.flux <- apply(eqn.vars.2, 1, 
                            function(x) { 
                              do.equation(x['alpha'], x['net.umol.unlab'], x['uptake.rate'], x['growth.integral'], x['product.integral']) } )

eqn.vars.2 <- eqn.vars.2 %>% mutate(resulting.aa.flux = alpha*umol.prot.flux)
